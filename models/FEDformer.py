...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.version == 'Wavelets':
            ...
            encoder_self_att = FourierBlock(configs, in_channels=configs.d_model,
                                            out_channels=configs.d_model,
                                            seq_len=self.seq_len,
                                            modes=configs.modes,
                                            mode_select_method=configs.mode_select)
            decoder_self_att = FourierBlock(configs, in_channels=configs.d_model,
                                            out_channels=configs.d_model,
                                            seq_len=self.label_len+self.pred_len,
                                            modes=configs.modes,
                                            mode_select_method=configs.mode_select)
            decoder_cross_att = FourierCrossAttention(configs, in_channels=configs.d_model,
                                                      out_channels=configs.d_model,
                                                      seq_len_q=self.label_len+self.pred_len,
                                                      seq_len_kv=self.seq_len,
                                                      modes=configs.modes,
        ...
        dec_modes = int(min(configs.modes, (configs.label_len+configs.pred_len)//2))
